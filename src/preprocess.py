...
def decode_input(input_data, save_file=True, return_file=True):
    ...
    if save_file:
        joblib.dump(output_data, "output_data_decode_v1.pickle")
...
def map_incomeclass_and_continents(df, column_name, type,save_file=True, return_file=True):
    ...
    if type == 'continents':
        ...
        joblib.dump(output, "output/continent_v1.pickle")
    if type == 'income_class':
        output = selectrows(df, column_name, income_class)
        joblib.dump(output, "output/income_class_v1.pickle")
...
def split_input_output(dataset, target_column, save_file=True, return_file=True):
    ...
    if save_file:
        joblib.dump(input_df, "output/input_df_v1.pickle")
        joblib.dump(output_df, "output/output_df_v1.pickle")
...
def split_train_validation(input_df, output_df, save_file=True, return_file=True, test_size=0.2):
    ...
    #Bagian dump ini bisa tidak diikutkan
    if save_file:
        joblib.dump(X_train, "output/X_train.pickle")
        joblib.dump(X_validation, "output/X_validation.pickle")
        joblib.dump(y_train, "output/y_train.pickle")
        joblib.dump(y_validation, "output/y_validation.pickle")
